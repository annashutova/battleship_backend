...
class Cell:
    ...
    def __str__(self):
        return Cell(self.state).name  # 'eg. "EMPTY"'

    def __repr__(self):
        return f"({self.x}, {self.y}), STATE: {CellStatus(self.state).name}" 
...
class Field(object):
    ...
    def __init__(self, size):
        ...
        self.radar = [[Cell(x, y) for x in range(size)] for y in range(size)]
        ...
        for p_x in range(x - 1, x + height + 1):
            for p_y in range(y - 1, y + width + 1):
                if p_x < 0 or p_x >= len(field) or p_y < 0 or p_y >= len(field):
                    continue
                if field[p_x][p_y] in (Cell.SHIP, Cell.DESTROYED):
                    return False
    ...
    def mark_destroyed_ship(self, ship, element):
        ...
        for p_x in range(x - 1, x + height + 1):
            for p_y in range(y - 1, y + width + 1):
                if p_x < 0 or p_x >= len(field) or p_y < 0 or p_y >= len(field):
                    continue
                field[p_x][p_y] = Cell.MISS

        for p_x in range(x, x + height):
            for p_y in range(y, y + width):
                field[p_x][p_y] = Cell.DESTROYED
...
if __name__ == '__main__':
    ...
    players = []
    players.append(Player(name='Username', is_ai=False, auto_ship=False))
